...
begin
  ...
  if not Assigned(DragDropEvent) then
    Exit;
  if QDropEvent_provides(DragDropEvent, PAnsiChar(Format)) then
  begin
    try
      ...
      try
        ...
        QDropEvent_encodedData(DragDropEvent, ByteArray, PAnsiChar(Format));
...
begin
  ...
  try
    ...
    try
      ...
      QClxDragObject_addFormat(ClxDragObj, PAnsiChar(Format), ByteArray);
    finally
      QIODevice_destroy(IO);
      if Assigned(ByteArray) then
        QByteArray_destroy(ByteArray);
...
{$ENDIF}
  ...
  begin
    ...
    GetCursorPos(Pt);
    Widget := FindControl(Pt);
    if not Assigned(Widget) then
      Exit;
    Control := Widget.ControlAtPos(Widget.ScreenToClient(Pt), False, False);
    ...
    case EventType of
      ...
      begin
        ...
        begin
          ...
          begin
            EnterPt := Pt; 
            EnterPt := LastDragOverControl.ScreenToClient(EnterPt);
            LastDragOverControl.DragOver(SendDragObject, EnterPt.X,
              EnterPt.Y, DragState, DragAccepted);
          end;
        end
        ...
        begin
          ...
          begin
            ...
            EnterPt := Control.ScreenToClient(Pt);
            Control.DragOver(SendDragObject, EnterPt.X, EnterPt.Y, DragState,
              DragAccepted);
          end;
        end;
        case EventType of
      ...
      begin
        ...
        begin
          EnterPt := Pt;
          EnterPt := Control.ScreenToClient(EnterPt);
          Control.DragOver(SendDragObject, EnterPt.X, EnterPt.Y,
                           dsDragLeave, Result);
        end;
        if Result then
          Control.DragDrop(SendDragObject, EnterPt.X, EnterPt.Y);
        DragDone(Result);
        Result := True;
      end;
    end;
